module Docrb
  class CommentParser
    ...
    def parse(data)
      ...
      if [:def, :sdef].include? @type
        infer_visibility_from_doc
      end
    ...
    def infer_visibility_from_doc
      ...
      return unless (annotation = VISIBILITY_ANNOTATIONS.find { |k, v| v.match?(item.text) })
    ...
    def load(text)
      text.each_char do |c|
        @last_char, @current_char = @current_char, c
        next if c == CARRIAGE
        if @last_char == LINE_BREAK
          if c == LINE_BREAK
            commit_current!
            next
          end
    ...
    def detect_text_references(text)
      changed = true
      while changed
        text, changed = update_next_reference(text)
      end
      text
    end
    ...
    def detect_field_references(field)
      field
        .to_a
        .collect { |k, v| [k, { type: :text_block, contents: detect_text_references(v) } ] }
        .to_h
    end
    ...
    def update_next_reference(contents)
      if contents.is_a? String
        if (match = COMMENT_METHOD_REF_REGEXP.match(contents))
          ...
          new_val = [
            { type: :span, contents: left_slice },
            { type: :camelcase_identifier, contents: match[0] },
            { type: :span, contents: right_slice }
          ].reject { |i| i[:contents].empty? }
          return new_val, true
        end
