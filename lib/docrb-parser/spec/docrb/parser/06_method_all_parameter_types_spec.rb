...
RSpec.describe Docrb::Parser do
  it "parses all parameter types" do
    context = parse_fixture(6)
    ...
    expect(method.parameters[3]).to be_positional & be_rest & be_named(:d)
    ...
    expect(method.parameters[6]).to be_keyword & be_rest & be_named(:g)
    expect(method.parameters[7]).to be_block & be_named(:h)
