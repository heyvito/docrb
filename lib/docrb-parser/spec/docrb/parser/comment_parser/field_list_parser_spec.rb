...
RSpec.describe Docrb::Parser::CommentParser::FieldListParser do
  it "parses a single line" do
    parser = described_class.new("field - Here goes docs")
    expect(parser.detect).to eq true
    expect(parser.result.transform_values { _1.map(&:to_h) }).to eq({
      "field" => [{ type: :text_block, value: "Here goes docs" }]
    })
  end
  ...
  it "parses lines with continuations" do
    ...
    expect(parser.detect).to eq true
    expect(parser.result.transform_values { _1.map(&:to_h) }).to eq({
      "field" => [{ type: :text_block, value: "Here goes docs that spans multiple lines" }]
    })
  end
  ...
  it "parses multiple fields" do
    ...
    expect(parser.detect).to eq true
    expect(parser.result.transform_values { _1.map(&:to_h) }).to eq({
      "field_a" => [{ type: :text_block, value: "Field A docs" }],
      "field_b" => [{ type: :text_block, value: "Field B docs" }]
    })
  end
  ...
  it "parses multiple continuations" do
    ...
    expect(parser.detect).to eq true
    expect(parser.result.transform_values { _1.map(&:to_h) }).to eq({
      "field" => [{ type: :text_block, value: "Here goes docs that spans multiple lines and multiple lines" }],
    })
  end
  ...
  it "parses multiple lines" do
    ...
    expect(parser.result.transform_values { _1.map(&:to_h) }).to eq({
      ...
      "level" => [{
        type: :text_block,
        value: "The level filter for the instance. Valid values are :error, :fatal, :info, :warn, and :debug"
      }],
      "format:" => [{
        type: :text_block,
        value: "The format to output logs. Supports :text and :json."
      }]
    })
  end
end
